...
class GT511_Fingerprint {
 public:
  GT511_Fingerprint(HardwareSerial *);

  void begin(uint16_t baud);

  boolean vrfypass(void);
  uint8_t retriveimg(void);
  ...
  void wrtpackt(uint32_t addr, uint8_t packttype, uint16_t len, uint8_t *packt);
  uint8_t retriveresponce(uint8_t packt[], uint16_t timeout=DEFAULTTIMEOUT);
